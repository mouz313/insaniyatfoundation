...
@section('content_header')
    <div class="d-flex align-items-center">
        <div class="mr-3" style="width: 56px; height: 56px; background: linear-gradient(135deg, #ffc107, #e0a800); border-radius: 12px; display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 15px rgba(255,193,7,0.3);">
            <i class="fas fa-user-edit text-white" style="font-size: 24px;"></i>
        </div>
        <div>
            <h1 class="mb-0" style="font-weight: 600;">Edit Donor: {{ $donor->name }}</h1>
            <small class="text-muted">Reg #: {{ $donor->registration_no ?? 'N/A' }}</small>
        </div>
    </div>
@stop
    ...
    <form action="{{ route('admin.donors.update', $donor->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="card shadow-sm border-0 rounded-lg mb-4" style="border-top: 3px solid #dc3545 !important;">
            <div class="card-header bg-white">
                <h5 class="mb-0"><i class="fas fa-user text-danger"></i> Personal Information</h5>
            </div>
            <div class="card-body">
                <div class="row">
                    <div class="col-md-6">
                        <x-adminlte-input name="name" label="Full Name" placeholder="Enter full name" value="{{ old('name', $donor->name) }}" required/>
                    </div>
                    <div class="col-md-6">
                        <x-adminlte-input name="father_name" label="Father's Name" placeholder="Enter father's name" value="{{ old('father_name', $donor->father_name) }}"/>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-3">
                        <x-adminlte-input name="cnic" label="CNIC" placeholder="XXXXX-XXXXXXX-X" value="{{ old('cnic', $donor->cnic) }}"/>
                    </div>
                    <div class="col-md-3">
                        <x-adminlte-input name="phone" label="Phone Number" placeholder="03XX-XXXXXXX" value="{{ old('phone', $donor->phone) }}" required/>
                    </div>
                    <div class="col-md-3">
                        <x-adminlte-input name="dob" label="Date of Birth" type="date" value="{{ old('dob', $donor->dob ? $donor->dob->format('Y-m-d') : '') }}"/>
                    </div>
                    <div class="col-md-3">
                ...
                <div class="row">
                    ...
                    <div class="col-md-4">
                        <x-adminlte-select name="status" label="Status">
                            ...
                            <option value="ineligible" {{ old('status', $donor->status) == 'ineligible' ? 'selected' : '' }}>Ineligible</option>
                        </x-adminlte-select>
                    </div>
                </div>
            </div>
        ...
        <div class="card shadow-sm border-0 rounded-lg mb-4" style="border-top: 3px solid #17a2b8 !important;">
            <div class="card-header bg-white">
                <h5 class="mb-0"><i class="fas fa-map-marker-alt text-info"></i> Contact & Location</h5>
            </div>
            <div class="card-body">
                <x-adminlte-textarea name="address" label="Address" placeholder="Enter full address">{{ old('address', $donor->address) }}</x-adminlte-textarea>
                <div class="row">
                    <div class="col-md-6">
                            ...
                            <div class="input-group">
                                <select name="city_id" id="city_id" class="form-control" required>
                                    ...
                                    @foreach($cities as $city)
                                        <option value="{{ $city->id }}" {{ old('city_id', $donor->city_id) == $city->id ? 'selected' : '' }}>{{ $city->name }}</option>
                                    @endforeach
                                </select>
                                <div class="input-group-append">
                                    <button type="button" class="btn btn-outline-success" data-toggle="modal" data-target="#addCityModal">
                            ...
                            <div class="input-group">
                                <select name="area_id" id="area_id" class="form-control">
                                    ...
                                    @foreach($areas as $area)
                                        <option value="{{ $area->id }}" {{ old('area_id', $donor->area_id) == $area->id ? 'selected' : '' }}>{{ $area->name }}</option>
                                    @endforeach
                                </select>
                                <div class="input-group-append">
                                    <button type="button" class="btn btn-outline-success" data-toggle="modal" data-target="#addAreaModal">
                                        <i class="fas fa-plus"></i>
                                    </button>
                                </div>
                            </div>
        ...
        <div class="card shadow-sm border-0 rounded-lg mb-4" style="border-top: 3px solid #28a745 !important;">
            <div class="card-header bg-white">
                <h5 class="mb-0"><i class="fas fa-heartbeat text-success"></i> Health & Medical</h5>
            </div>
            <div class="card-body">
                <div class="row">
                    <div class="col-md-4">
                        <x-adminlte-input name="weight" label="Weight (kg)" type="number" step="0.1" min="30" max="300" placeholder="e.g. 65.5" value="{{ old('weight', $donor->weight) }}"/>
                    </div>
                    <div class="col-md-4">
                        <x-adminlte-input name="hemoglobin" label="Hemoglobin (g/dL)" type="number" step="0.1" min="5" max="20" placeholder="e.g. 13.5" value="{{ old('hemoglobin', $donor->hemoglobin) }}"/>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-5">
                        <x-adminlte-input name="last_donation_date" id="lastDonationDate" label="Last Donation Date" type="date" value="{{ old('last_donation_date', $donor->last_donation_date?->format('Y-m-d')) }}"/>
                    </div>
                    <div class="col-md-3">
                    ...
                    <div class="col-md-4">
                        <x-adminlte-input name="total_donations" label="Total Donations (Life Time)" type="number" min="0" value="{{ old('total_donations', $donor->total_donations ?? 0) }}"/>
        ...
        {{-- Section 4: Education & Referral --}}
        <div class="card shadow-sm border-0 rounded-lg mb-4" style="border-top: 3px solid #ffc107 !important;">
            <div class="card-header bg-white">
                <h5 class="mb-0"><i class="fas fa-graduation-cap text-warning"></i> Education & Referral</h5>
            </div>
            <div class="card-body">
                <div class="row">
                    <div class="col-md-4">
                        <x-adminlte-select name="education" label="Education">
                            ...
                            @foreach(['Matric', 'Intermediate', 'Bachelor\'s', 'Master\'s', 'PhD', 'Other'] as $edu)
                                <option value="{{ $edu }}" {{ old('education', $donor->education) == $edu ? 'selected' : '' }}>{{ $edu }}</option>
                            @endforeach
                        </x-adminlte-select>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <div class="custom-control custom-switch custom-switch-off-secondary custom-switch-on-success" style="padding-top: 6px;">
                                <input type="checkbox" class="custom-control-input" name="is_student" value="1" {{ old('is_student', $donor->is_student) ? 'checked' : '' }} id="is_student">
                                <label class="custom-control-label" for="is_student">Is Student</label>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-4" id="university_field_wrapper" style="{{ old('is_student', $donor->is_student) ? '' : 'display:none;' }}">
                        <div class="form-group">
                            ...
                            <div class="input-group">
                                <select name="university_id" id="university_id" class="form-control">
                                    ...
                                    @foreach($universities as $uni)
                                        <option value="{{ $uni->id }}" {{ old('university_id', $donor->university_id) == $uni->id ? 'selected' : '' }}>{{ $uni->name }}</option>
                                    @endforeach
                            ...
                            <input type="text" name="university_name" id="university_name_input" class="form-control mt-2" placeholder="Enter university name" value="{{ old('university_name', $donor->university_name) }}" style="{{ old('university_id', $donor->university_id) == 'new' ? '' : 'display:none;' }}">
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-8">
                        ...
                        <div class="input-group">
                            <input type="text" id="refSearch" class="form-control" placeholder="Search by CNIC or Phone (min 3 chars)">
                            <div class="input-group-append">
                        ...
                        <input type="hidden" name="referred_by" id="referredByInput" value="{{ old('referred_by', $donor->referred_by) }}">
                        <input type="hidden" name="no_referral" id="noReferralInput" value="0">
                    </div>
                    <div class="col-md-4" style="padding-top:38px;">
                        <div class="custom-control custom-checkbox">
                            <input type="checkbox" class="custom-control-input" name="no_referral_check" id="noReferral" value="1">
        ...
        <div class="card shadow-sm border-0 rounded-lg mb-4" style="border-top: 3px solid #6f42c1 !important;">
            <div class="card-header bg-white">
                <h5 class="mb-0"><i class="fas fa-camera text-purple"></i> Photo</h5>
            </div>
            <div class="card-body">
                <div class="form-group">
                    <div class="mb-2">
                        @if($donor->photo)
                            <img id="photoPreview" src="{{ asset('storage/' . $donor->photo) }}" alt="Preview" class="img-thumbnail" width="120" height="120" style="object-fit:cover;">
                        @else
        ...
        <div class="text-right mb-4">
            <x-adminlte-button type="submit" label="Update Donor" theme="warning" icon="fas fa-save"/>
            ...
            <div class="modal-content">
                ...
                <form id="addCityForm">
                    <div class="modal-body">
                        <div class="form-group">
                            <label for="newCityName">City Name <span class="text-danger">*</span></label>
                            <input type="text" id="newCityName" class="form-control" placeholder="Enter city name" required>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="submit" class="btn btn-success" id="saveCityBtn"><i class="fas fa-save"></i> Save</button>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form id="addAreaForm">
                    <div class="modal-body">
                        <div class="form-group">
                            <label for="newAreaCity">City <span class="text-danger">*</span></label>
                            <select id="newAreaCity" class="form-control" required>
                                <option value="">Select City</option>
                                @foreach($cities as $city)
                        ...
                        <div class="form-group">
                            <label for="newAreaName">Area Name <span class="text-danger">*</span></label>
                            <input type="text" id="newAreaName" class="form-control" placeholder="Enter area name" required>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="submit" class="btn btn-success" id="saveAreaBtn"><i class="fas fa-save"></i> Save</button>
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
                    </div>
                </form>
            </div>
...
<script>
    $(function() {
        ...
        function loadAreas(cityId, selectedAreaId) {
            ...
            $.get('{{ url("admin/areas/by-city") }}/' + cityId, function(data) {
                ...
                $.each(data, function(i, area) {
                    html += '<option value="' + area.id + '"' + (selectedAreaId == area.id ? ' selected' : '') + '>' + area.name + '</option>';
                });
                $('#area_id').html(html);
            });
        }
        ...
        @if(old('city_id', $donor->city_id))
            loadAreas({{ old('city_id', $donor->city_id) }}, {{ old('area_id', $donor->area_id) ?? 'null' }});
        @endif

        $('#is_student').on('change', function() {
            if ($(this).is(':checked')) {
                $('#university_field_wrapper').slideDown(200);
            } else {
                $('#university_field_wrapper').slideUp(200);
                $('#university_field_wrapper input').val('');
                $('#university_id').val('');
            }
        });
        ...
        var initialDate = $('#lastDonationDate').val();
        if (initialDate) {
            var d = new Date(initialDate + 'T00:00:00');
            var today = new Date();
            today.setHours(0,0,0,0);
            if (d <= today) $('#monthsAgo').val(monthsBetween(d, today));
        }
        ...
        {{-- Referrer search --}}
        var referredById = {{ old('referred_by', $donor->referred_by) ?: 'null' }};
        var referrerName = '{{ $donor->referrer ? addslashes($donor->referrer->name) : "" }}';
        var referrerCnic = '{{ $donor->referrer ? addslashes($donor->referrer->cnic) : "" }}';
        var referrerPhone = '{{ $donor->referrer ? addslashes($donor->referrer->phone) : "" }}';
        var referrerBlood = '{{ $donor->referrer ? addslashes($donor->referrer->blood_group) : "" }}';

        if (referredById) {
            selectReferrer(referredById, referrerName, referrerCnic, referrerPhone, referrerBlood);
        }
        ...
        $('#addCityForm').on('submit', function(e) {
            e.preventDefault();
            var name = $('#newCityName').val().trim();
            if (!name) return;
            $('#saveCityBtn').prop('disabled', true).html('<i class="fas fa-spinner fa-spin"></i> Saving...');
            $.ajax({
                ...
                success: function(res) {
                    if (res.success) {
                        $('#city_id').append('<option value="' + res.id + '" selected>' + res.name + '</option>');
                        $('#newAreaCity').append('<option value="' + res.id + '">' + res.name + '</option>');
                        toastr.success('City "' + res.name + '" added!', 'Success');
                        $('#addCityModal').modal('hide');
                        $('#newCityName').val('');
                        loadAreas(res.id, null);
                    }
                },
                error: function(xhr) {
                    var msg = xhr.responseJSON?.errors?.name?.[0] || 'Failed to add city';
                    toastr.error(msg, 'Error');
                },
                complete: function() {
                    $('#saveCityBtn').prop('disabled', false).html('<i class="fas fa-save"></i> Save');
                }
            });
        });
        ...
        $('#addAreaForm').on('submit', function(e) {
            e.preventDefault();
            var cityId = $('#newAreaCity').val();
            var name = $('#newAreaName').val().trim();
            if (!cityId || !name) return;
            $('#saveAreaBtn').prop('disabled', true).html('<i class="fas fa-spinner fa-spin"></i> Saving...');
            $.ajax({
                url: '{{ route("admin.areas.store") }}',
                method: 'POST',
                data: { city_id: cityId, name: name, _token: '{{ csrf_token() }}' },
                success: function(res) {
                    if (res.success) {
                        $('#area_id').append('<option value="' + res.id + '" selected>' + res.name + '</option>');
                        toastr.success('Area "' + res.name + '" added!', 'Success');
                        $('#addAreaModal').modal('hide');
                        $('#newAreaName').val('');
                    }
                },
                error: function(xhr) {
                    var msg = xhr.responseJSON?.errors?.name?.[0] || 'Failed to add area';
                    toastr.error(msg, 'Error');
                },
                complete: function() {
                    $('#saveAreaBtn').prop('disabled', false).html('<i class="fas fa-save"></i> Save');
                }
            });
        });

        $('#addCityModal, #addAreaModal').on('hidden.bs.modal', function() {
