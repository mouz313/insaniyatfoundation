...
.swiper-button-next, .swiper-button-prev { color: #fff; background: rgba(220,53,69,0.8); width: 40px; height: 40px; border-radius: 50%; }
.swiper-button-next::after, .swiper-button-prev::after { font-size: 16px; }
.swiper-button-next { right: 10px; }
.swiper-button-prev { left: 10px; }
.swiper-pagination-bullet { background: rgba(255,255,255,0.4); opacity: 1; }
.swiper-pagination-bullet-active { background: #dc3545; width: 24px; border-radius: 4px; }
</style>
@endpush
...
@section('content')
    <div class="container py-5">
        <div class="text-center mb-5">
            <div style="width:80px;height:80px;border-radius:20px;background:linear-gradient(135deg,#dc3545,#e4606d);display:flex;align-items:center;justify-content:center;margin:0 auto 20px;box-shadow:0 10px 30px rgba(220,53,69,0.3);">
                <i class="fas fa-tint text-white" style="font-size:36px;"></i>
            </div>
            <h1 style="font-weight:800;color:#1a1a2e;font-size:2.2rem;">Blood Availability</h1>
            <p class="text-muted" style="font-size:1.05rem;">Find active donors by blood group and city</p>
        </div>

        {{-- Carousel --}}
        @if($carouselDonors->count() > 0)
            <div class="mb-5" style="border-radius:24px;background:linear-gradient(135deg,#1a1a2e 0%,#16213e 50%,#0f3460 100%);padding:3px;">
                <div style="border-radius:22px;background:linear-gradient(135deg,rgba(255,255,255,0.08),rgba(255,255,255,0.02));padding:30px 50px 50px;">
                    <div class="d-flex align-items-center mb-4 px-3">
                        <span style="background:rgba(220,53,69,0.2);color:#ff6b7a;padding:4px 14px;border-radius:20px;font-size:12px;font-weight:600;letter-spacing:1px;"><i class="fas fa-crown mr-1"></i> FEATURED DONORS</span>
                    </div>
                    <div class="swiper">
                                    ...
                                    <div class="row align-items-center">
                                        <div class="col-md-5 text-center mb-4 mb-md-0">
                                            <div style="position:relative;display:inline-block;">
                                                <div style="width:140px;height:140px;border-radius:50%;background:linear-gradient(135deg,#dc3545,#e4606d);display:flex;align-items:center;justify-content:center;font-weight:700;font-size:52px;color:#fff;border:4px solid rgba(255,255,255,0.2);box-shadow:0 15px 40px rgba(220,53,69,0.4);margin:0 auto;">
                                                ...
                                                <div style="position:absolute;bottom:5px;right:5px;background:#ffc107;border-radius:50%;width:36px;height:36px;display:flex;align-items:center;justify-content:center;border:3px solid #1a1a2e;box-shadow:0 4px 10px rgba(0,0,0,0.3);">
                                                    <i class="fas fa-tint" style="color:#dc3545;font-size:14px;"></i>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="col-md-7 text-center text-md-left">
                                            <h3 style="font-weight:800;color:#fff;font-size:1.6rem;margin-bottom:4px;">{{ $d->name }}</h3>
                                            <span style="background:#dc3545;color:#fff;padding:4px 18px;border-radius:20px;font-size:15px;font-weight:700;display:inline-block;margin-bottom:14px;"><i class="fas fa-tint mr-1"></i> {{ $d->blood_group }}</span>
        ...
        <div class="row mb-4">
            ...
            <div class="col-md-4">
                <div class="card shadow-sm border-0 rounded-lg h-100 stat-card" style="border-left:4px solid #28a745;">
                    <div class="card-body d-flex align-items-center">
        ...
        {{-- Search Form --}}
        <div class="card shadow-sm border-0 rounded-lg mb-4" style="border-top:4px solid #dc3545;">
                ...
                <form method="GET" action="{{ route('portal.availability') }}" id="availabilityForm">
                    ...
                    <div class="row align-items-end">
                        <div class="col-md-5 mb-3 mb-md-0">
                            ...
                            <select name="blood_group" class="form-control rounded-pill" style="border-color:#dee2e6;">
                        ...
                        <div class="col-md-5 mb-3 mb-md-0">
                            ...
                            <select name="city_id" class="form-control rounded-pill" style="border-color:#dee2e6;">
                        ...
                        <div class="col-md-2 d-flex gap-2">
                            <button type="submit" class="btn btn-danger btn-block rounded-pill py-2" style="font-weight:600;flex:1;">
        ...
        @if(isset($donors))
            <div class="d-flex align-items-center mb-3">
                <div style="width:36px;height:36px;border-radius:10px;background:linear-gradient(135deg,#dc3545,#e4606d);display:flex;align-items:center;justify-content:center;margin-right:12px;">
                ...
                <div class="row">
                    @foreach($donors as $donor)
                        <div class="col-md-6 col-lg-4 mb-4">
                            <div class="card shadow-sm border-0 rounded-lg h-100" style="transition:transform 0.2s;border-left:4px solid #dc3545;">
                                <div class="card-body">
                                    <div class="d-flex align-items-center mb-3">
                                        <div style="width:48px;height:48px;border-radius:50%;background:linear-gradient(135deg,#dc3545,#e4606d);display:flex;align-items:center;justify-content:center;flex-shrink:0;font-weight:700;font-size:18px;color:#fff;">
                                            {{ strtoupper(substr($donor->name, 0, 1)) }}
                                        </div>
                                        <div class="ml-3">
                                            <h6 class="mb-0" style="font-weight:700;">{{ $donor->name }}</h6>
                                            <small class="text-muted"><i class="fas fa-phone-alt mr-1" style="width:14px;"></i>{{ $donor->phone }}</small>
                                        </div>
                                    </div>
                                    <div class="d-flex flex-wrap" style="gap:8px;">
                                        <span class="badge rounded-pill px-3 py-2" style="background:#dc3545;color:#fff;font-size:13px;font-weight:600;">
                                        ...
                                        <span class="badge rounded-pill px-3 py-2" style="background:#f8f9fa;color:#495057;font-size:13px;font-weight:500;border:1px solid #dee2e6;">
                                            <i class="fas fa-map-marker-alt mr-1 text-danger"></i>{{ $donor->city->name ?? 'N/A' }}
                                        </span>
                                        @if($donor->area)
                                            <span class="badge rounded-pill px-3 py-2" style="background:#f8f9fa;color:#495057;font-size:13px;font-weight:500;border:1px solid #dee2e6;">
                                                <i class="fas fa-map-pin mr-1 text-danger"></i>{{ $donor->area->name }}
                                            </span>
                                        @endif
                                        <span class="badge rounded-pill px-3 py-2" style="background:#f8f9fa;color:#495057;font-size:13px;font-weight:500;border:1px solid #dee2e6;">
                                            <i class="fas fa-calendar mr-1 text-danger"></i>{{ $donor->last_donation_date ? \Carbon\Carbon::parse($donor->last_donation_date)->format('d M Y') : 'Never' }}
                                        </span>
                                        @if($donor->total_donations > 0)
                                            <span class="badge rounded-pill px-3 py-2" style="background:#28a745;color:#fff;font-size:13px;font-weight:600;">
                                                <i class="fas fa-heart mr-1"></i>{{ $donor->total_donations }} donation{{ $donor->total_donations !== 1 ? 's' : '' }}
                                            </span>
                                        @endif
                                    </div>
                                </div>
